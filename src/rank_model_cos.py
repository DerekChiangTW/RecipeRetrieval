...
class img2img_feat(nn.Module):
    def __init__(self, num_ingr=3, imfeatDim=2048, embDim=1024):
        ...
        self.visual_embedding1 = nn.Sequential(nn.Linear(imfeatDim, embDim), nn.Tanh(),nn.Dropout(),nn.Linear(embDim, embDim), nn.Tanh())
        ...
        #self.score = nn.Sequential(nn.Linear(2*embDim,embDim),nn.Tanh(),nn.Dropout(),nn.Linear(embDim,1))
    ...
    def forward(self, dish_feat, ingr_feats):
        ...
        output = self.score(ingr_embs,dish_emb)
        return output
