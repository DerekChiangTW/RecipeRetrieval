...
class ImagerLoader(data.Dataset):
    ...
    def __getrecipe__(self,index):
        recipeId = self.ids[index]

        # get the recipe from the lmdb
        with self.env.begin(write=False) as txn:
            serialized_sample = txn.get(recipeId)
        sample = pickle.loads(serialized_sample)
        ...
        if self.partition == 'train':
            if self.semantic_reg:
                return [img, instrs, itr_ln, ingrs, igr_ln], [img_class, rec_class]
            else:
                return [img, instrs, itr_ln, ingrs, igr_ln], []
        else:
            if self.semantic_reg:
                return [img, instrs, itr_ln, ingrs, igr_ln], [img_class, rec_class, img_id, rec_id]
            else:
                return [img, instrs, itr_ln, ingrs, igr_ln], [img_id, rec_id]
